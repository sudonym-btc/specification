...
Generic marketplace behavior, including `autoAccept`, `negotiable`,
`quantity`, minimum rentable duration, security deposits, minimum payment
amounts, maximum dispute periods, and cancellation policies, is defined by the
NIP-99 Marketplace Listing Extension. This NIP only defines
accommodation-specific profile tags and location indexing.
...
`I` for `autoAccept` and `N` for `negotiable`, retain their generic meaning.
